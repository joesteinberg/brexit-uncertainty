  ...
  for(i=0; i<solver_n; i++)
    {
      ...
      for(j=0; j<solver_n; j++)
	{
	  fprintf(logfile, "%0.1e",gsl_matrix_get(J,i,j));
	  if(j==solver_n-1)
	    {
	      fprintf(logfile,"\n");
	    }
  ...
  if(status)
  {
      ...
      if(status == GSL_CONTINUE)
	{
	  do
		...
		{
		  status = GSL_EFAILED;
		  fprintf(logfile,KRED "\t\tdx is zero vector but equations not solved!\n" RESET);
		  fprintf(logfile,KRED "\t\tmaxloc(f) = %d, minloc(f) = %d\n" RESET,
			  (int)gsl_vector_max_index(s->f),(int)gsl_vector_min_index(s->f));
		  break;
		}
	      ...
	      if(solver_verbose)
		{
		  if(f->n <= 4)
		    ...
		    {
		      fprintf(logfile,
			      KMAG "\t\t%2d, %2.0f sec, max(f) = %+0.1e, min(f) = %+0.1e, max(dx) = %+0.1e, min(dx) = %+0.1e\n" RESET,
			      iter,difftime(stop2,start2),
			      gsl_vector_max(s->f),
			      gsl_vector_min(s->f),
			      gsl_vector_max(s->dx),
			      gsl_vector_min(s->dx));
