...
uint find_root_deriv(gsl_multiroot_function_fdf * f)
{
  ...
  jcnt=0;
  time(&start2);
  status = gsl_multiroot_fdfsolver_set (s, f, solver_x);
  time(&stop2);
  ...
  if(status)
  {
  ...
  else
    {
      ...
      fprintf(logfile,KMAG "\t\tInitialization complete. Time = %0.2f, nj = %d\n" RESET,difftime(stop2,start2),jcnt);
      ...
      if(status == GSL_CONTINUE)
	{
	  do
	    {
	      ...
	      //status = gnewton_iterate(s->state,f,s->x,s->f,s->J,s->dx);
	      ...
	      if(status != GSL_SUCCESS && gsl_vector_max(s->dx) < 1.0e-14 && gsl_vector_min(s->dx) > -1.0e-14)
		{
	      ...
	      if(f->n <= 4)
		{
		  fprintf(logfile,KMAG "\t\tIter %d, time = %0.2f, f = (" RESET,iter,difftime(stop2,start2));
		  uint i;
		  for(i=0; i<f->n; i++)
		    {
		...
		{
		  fprintf(logfile,KMAG "\t\tIter %d\ttime = %0.2f\tnj = %d\tmax(f) = %0.2e\tmin(f) = %0.2e\n" RESET,
			  iter,
      ...
      double sum = cblas_dasum(f->n,(s->f)->data,1);
